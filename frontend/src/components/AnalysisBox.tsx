...
export default function AnalysisBox({ checklist }: Props) {
  ...
  default: {
   ...
   const { type, description, advice } = map[code] ?? map.default;
    return { code, type, description, advice };
  }, [checklist]);
  ...
  return (
    <div className={pageStyles.analysisBox}>
      <h3>분석 결과</h3>
       <div className={pageStyles.analysisCode}>
        <strong>피부 유형: </strong>{analysis.code}
      </div>
